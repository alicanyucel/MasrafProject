...
{
    internal sealed class CreateApprovalStatusCommandHandler(IApprovalStatusRepository approvalStatusRepository, IUnitOfWork unitOfWork, IMapper mapper) : IRequestHandler<CreateApprovalStatusCommand, Result<string>>
    {
        public async Task<Result<string>> Handle(CreateApprovalStatusCommand request, CancellationToken cancellationToken)
        {
            ...
            return "Approval Status kaydı yapıldı";
        }
    }
}